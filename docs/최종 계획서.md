...
| `omx_rl_control` | 상세 런타임 설계와 모델용 디렉터리만 존재, 실행 파일은 비어 있음 | 구현 대기 | 모델 배포, 33D 관측, PPO 추론, trajectory·gripper 구현 |
...
`omx_turtle.launch.py`는 아직 `mp_control`, `aruco_mp_bridge`, MoveIt Servo를 사용하는 legacy 팔 제어 경로를 포함한다. 이 경로는 `omx_rl_control`이 구현될 때 **교체**하며 두 팔 제어기를 동시에 실행하지 않는다. 또한 `turtlebot3_position`이 아직 없으므로 현재 `nav_cmd_vel`을 생성하는 최종 주행 구현도 없다.
...
| 모델 크기 | `1,842,083 bytes` |
| SHA-256 | `d838bab2c6b034252cdf10e52d52a9a77de3bc80f7c9c41ea82554f1b8aa50f2` |
...
| Pick/Place 혼합 | 98% | 2% |
| 전체 Sim2Real randomization | 92% | 7% |
...
- 런타임은 `/home/ktj/omx_train_ws`를 직접 참조하지 않는다. 검증된 버전을 `omx_rl_control/models/policies/arm_delivery_residual_v2/`에 배포한다.
...
| 예정 | `/rl_control/command` | 구현 전 확정 | Coordinator → RL | PICK, STAY_HOLD, PLACE, STAY_EMPTY 요청 |
| 예정 | `/rl_control/status` | 구현 전 확정 | RL → Coordinator | 단계 완료와 오류 코드 |
| 예정 | `/safety_stop` | `std_msgs/msg/Bool` | Safety → Mux·RL | 출력 즉시 차단 |

`/rl_control/command`와 `/rl_control/status`는 코드 작성 전에 메시지 타입과 완료·오류 코드를 확정한다. 자유 형식 문자열을 장기 계약으로 사용하지 않는다.
...
| P3. RL 배포 | 정책·metadata 고정, 관측 builder, 추론, 관절 제한 구현 | 고정 입력 출력 재현, checksum·33D 순서 검증 | 대기 |
| P4. Fake 통합 | fake ros2_control에서 Vision replay → 파지 → Stay → 배치 | 제어권 충돌 없이 전체 팔 상태 전이 완료 | 대기 |
...
3. 최종 PPO ZIP과 metadata를 runtime 패키지에 버전 고정하고 모델 계약 검증 코드를 작성한다.
4. `omx_rl_control`의 33D 관측 생성, 4D 추론, 관절 제한, 그리퍼 상태 머신을 구현한다.
5. fake hardware에서 legacy `mp_control`을 RL 경로로 교체한다.
