...
| ROS 2 실행 노드 | `rl_control_node.py`가 0 byte | 미구현 |
| launch·YAML | `rl_control.launch.py`, `rl_control.yaml`이 0 byte | 미구현 |
| console entry point | `setup.py`에 없음 | 미구현 |
| 정책 배포 | runtime 패키지에 PPO ZIP·metadata 없음 | 미구현 |
| 33D observation parity | 학습 코드만 있고 ROS parity 시험 없음 | 미구현 |
| reference controller parity | 학습 환경 구현만 있고 ROS 구현 없음 | 미구현 |
| fake hardware | 전체 상태 전이 시험 없음 | 미진행 |
...
현재 metadata의 `model_sha256`은 PPO ZIP이 아니라 학습 MuJoCo 모델의 checksum이다. 배포본에는 오해가 없도록 `policy_sha256`, `mujoco_model_sha256`, `training_config_sha256`를 구분한 manifest를 추가한다.
...
stateDiagram-v2
    [*] --> NOT_READY
    NOT_READY --> STAY_EMPTY: artifact / TF / controller valid
    ...
    VERIFY_GRASP --> RECOVERY: grasp failed
    HOLD --> RECOVERY: timeout
    RECOVERY --> STAY_EMPTY: manual reset and safe return
    NOT_READY --> E_STOP: safety stop
    PICK_REACH --> E_STOP: safety stop
    PLACE_REACH --> E_STOP: safety stop
    E_STOP --> NOT_READY: manual release
```

| 상태 | 정책 phase | 그리퍼 | 베이스 |
|---|---|---|---|
| `STAY_EMPTY` | 비활성 | Open 또는 안전 상태 | Hold |
...
| 출력 | `/rl_control/status` | 구현 전 확정 | phase, 완료, 오류 코드 |

PICK·PLACE는 취소와 timeout이 필요한 장시간 작업이므로 최종 명령 인터페이스는 ROS 2 Action을 우선 검토한다. 구현 전 `omx_interfaces` 생성 여부를 결정하고, 자유 형식 문자열을 장기 계약으로 고정하지 않는다.
...
| `state_machine.py` | Pick, Stay, Place, Hold, Recovery, E-Stop 전이 |
...
| 단계 | 작업 | 완료 기준 | 상태 |
|---|---|---|---|
| R0 | 정책 배포 manifest 작성 | 정책·MJCF·config checksum 의미가 분리됨 | 대기 |
| R1 | 학습 golden vector export | observation, raw action, reference, final target 표본 확보 | 대기 |
| R2 | model contract·observation builder | 33D 순서·정규화가 golden vector와 일치 | 대기 |
| R3 | reference·action limiter | 기준 action과 최종 관절 target이 학습 코드와 일치 | 대기 |
| R4 | state machine·gripper | PPO 없이 Open, Close, Hold, Release, Stay 전이 통과 | 대기 |
| R5 | fake ros2_control | Pick·Place 전체 전이와 timeout·E-Stop 통과 | 대기 |
...
1. 정책 ZIP용 `SHA256SUMS`와 명확한 artifact manifest를 만든다.
2. 학습 환경에서 phase별 golden observation·action 표본을 export한다.
3. 정책, metadata, config, 평가 YAML을 runtime 모델 디렉터리에 버전 고정한다.
4. `model_contract.py`와 `observation_builder.py`부터 구현한다.
5. reference waypoint와 pregrasp IK를 학습 코드와 parity 시험한다.
6. action limiter와 50 Hz inference loop를 fake hardware에 연결한다.
7. 결정론적 그리퍼와 Pick/Place 상태 머신을 추가한다.
8. Vision rosbag으로 dropout·target latch·Hold 전이를 검증한다.
9. legacy `mp_control`을 비활성화한 뒤 저속 실기기 시험을 시작한다.
